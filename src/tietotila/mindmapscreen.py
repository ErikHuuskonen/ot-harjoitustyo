...
class DeckView(tk.Toplevel):
    ...
    def build_widgets(self):
        ...
        """
        self.clear_widgets()
        self.add_button = tk.Button(self, text="Lisää", command=self.add_card_view)
        ...
        self.show_answer_button = tk.Button(self, text="Näytä vastaus", bg="blue", fg="blue", command=self.show_answer)
        self.show_answer_button.grid(row=3, column=0, columnspan=2, sticky='nsew')
        self.card_label = tk.Label(self, wraplength=500, height=10, font=("Arial", 22))
        self.card_label.grid(row=2, column=0, columnspan=2, sticky='nsew')
        self.grid_rowconfigure(1, weight=1, pad=5)
        self.grid_columnconfigure(1, weight=1, pad=5)
        if len(self.deck.cards) > 0:
            self.card_label.config(text=self.deck.cards[self.card_index].back_text)
        else:
    ...
    def add_card_view(self):
        ...
        """
        self.clear_widgets()
        self.back_button = tk.Button(self, text="Takaisin", command=self.build_widgets)
        ...
        self.front_entry = tk.Text(self, bg="white", fg="black", insertbackground="black", height=2, bd=0, highlightthickness=1, highlightbackground="black") #muutos
        ...
        self.back_entry = tk.Text(self, bg="white", fg="black", insertbackground="black", height=2, bd=0, highlightthickness=1, highlightbackground="black") #muutos
        self.back_entry.grid(row=2, column=1, sticky='ew')
        self.save_button = tk.Button(self, text="Tallenna", command=self.save_card)
    ...
    def update_deck_view(self):
        ...
        if len(self.deck.cards) == 0:
            ...
            if self.current_card_index < len(self.deck.cards):
                self.card_label.config(text=self.deck.cards[self.current_card_index].front_text)
        self.update()
    def save_card(self):
        """
    ...
    def show_answer(self):
        ...
        if not self.showing_answer:
            ...
            self.pass_button = tk.Button(self, text="Ohi", command=lambda: self.next_card("pass"))
            self.pass_button.grid(row=3, column=0, sticky='ew')
            self.dont_remember_button = tk.Button(self, text="En muista", command=lambda: self.next_card("dont_remember"))
            self.dont_remember_button.grid(row=3, column=1, sticky='ew')
            self.almost_button = tk.Button(self, text="Melkein", command=lambda: self.next_card("almost"))
            self.almost_button.grid(row=4, column=0, sticky='ew')
            self.remember_button = tk.Button(self, text="Muistin", command=lambda: self.next_card("remember"))
    ...
    def next_card(self, answer_quality):
        ...
        self.update_deck_view()
        self.card_label.config(text=self.deck.cards[self.card_index].front_text)  # Muuta tämä rivi
